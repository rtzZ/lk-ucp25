...
_temp_codes: dict[str, tuple[str, float]] = {}  # user_id -> (code, expires_at)
...
async def telegram_request_code(body: TelegramLoginIn, session: AsyncSession = Depends(get_session)):
    ...
    # Проверяем, что пользователь состоит в группе через Telegram Bot API
    try:
        ...
        chat_member_url = (
            ...
            f"getChatMember?chat_id={_TELEGRAM_GROUP_ID}&user_id={student.telegram_user_id}"
        )
        ...
        _temp_codes[str(student.telegram_user_id)] = (code, time.time() + 60)  # 60 секунд TTL
